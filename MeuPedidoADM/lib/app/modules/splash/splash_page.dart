...
class _SplashPageState extends State<SplashPage> {
  ...
  Future<void> carregaEmpresaAtiva() async {
    // if (widget.identificador == null || widget.identificador == '') {
      ...
      var pj = '';
      pj = ((_authController.userAtual?.cnpjPadrao ?? '').isNotEmpty)
          ? _authController.userAtual.cnpjPadrao
          : (_authController.userAtual.empresas.length > 0)
              ? _authController.userAtual.empresas[0].cnpjM.docId
              : null;

      if (pj != null) {
        _cnpjsController.cnpjAtivo = await _cnpjsController.getCnpjM(pj);
